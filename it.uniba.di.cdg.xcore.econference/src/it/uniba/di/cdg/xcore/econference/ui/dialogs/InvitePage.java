...
{	
	...
	public void createControl(Composite parent) {
		...
		Iterator it = contatti.iterator();
		fromBuddy = new Table(composite,SWT.MULTI);
        gd = new GridData(200,250);
        fromBuddy.setLayoutData(gd);
        imageRegistry = new ImageRegistry();
        ImageDescriptor activeIcon =  UiPlugin.getDefault().getImageDescriptor( IImageResources.ICON_USER_ACTIVE );
        ImageDescriptor busyIcon =  UiPlugin.getDefault().getImageDescriptor( IImageResources.ICON_USER_BUSY );
        ImageDescriptor awayIcon =  UiPlugin.getDefault().getImageDescriptor( IImageResources.ICON_USER_AWAY );
        ImageDescriptor offlineIcon =  UiPlugin.getDefault().getImageDescriptor( IImageResources.ICON_USER_OFFLINE );
        ...
        while (it.hasNext()){
        	...
        	String nextID = buddy.getId();
        	TableItem item = new TableItem(fromBuddy, SWT.NULL);
        	item.setImage(getImage(buddy));
        	item.setText((nextName.equals("") ? buddy.getId() : buddy.getName()+" - "+buddy.getId()));
        }
        ...
        removeMember.addMouseListener(new org.eclipse.swt.events.MouseListener(){
			public void mouseDoubleClick(MouseEvent e) {}
			public void mouseDown(MouseEvent e) {}
			@Override
			public void mouseUp(MouseEvent e) {
        ...
        removeMember.pack();
        toConference = new Table(composite,SWT.MULTI);  
        gd = new GridData(200,250);
