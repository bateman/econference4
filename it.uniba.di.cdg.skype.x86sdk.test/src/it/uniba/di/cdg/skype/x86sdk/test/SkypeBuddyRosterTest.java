...
public class SkypeBuddyRosterTest extends TestCase {
	...
	public void testAddContainsBuddy() {
		...
		for (int i = 0; i < g.length; i++) {
			...
			if (group.GetStrProperty(ContactGroup.PROPERTY.given_displayname).equals("amici")){
				Contact[] friends=group.GetContacts();

				for (int j = 0; j < group.GetContacts().length; j++) {
					Contact friend = friends[j];

					if (friend.GetStrProperty(Contact.PROPERTY.skypename).equals("echo123"))
						trovato=true;
				}
			}
		}
	...
	public void testRemoveGroup() {       
		...
		for (int i = 0; i < g.length; i++) {
			...
			if (group.GetStrProperty(ContactGroup.PROPERTY.given_displayname).equals("famiglia")){
				trovato2=true;
			}
		}
